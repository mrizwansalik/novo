...
export function formatDefaultValue(carrierPlan: ICarrierPlan) {
  const formDefaultValue = {
    ...
    copay_office_visit: get(carrierPlan, "copay_office_visit", 0) * 100,
...
export function handleGoNextPage(history, pathname, brokerageId, prospectId) {
  if (pathname.includes("onboarding")) {
    history.push(
      routes.dashboard.brokerage.prospects.onBoarding.existingPlans.participation.getValue(
        prospectId
      )
    );
  } else {
    history.push(
      routes.dashboard.brokerage.brokerageId.prospects.prospectId.plans.selfFunded.participation.getValue(
        brokerageId,
        prospectId
      )
    );
