...
function usePlanDesignForm(carrierPlan: ICarrierPlan) {
  ...
  async function handleCarrierOptions(orgId: string) {
    const stopLossCarriers: ICarrier[] = await listOrgCarriers(orgId);
    const options =
  ...
  async function handleAddCarrierOptions(orgId: string, carrierName: string) {
    ...
    const newCarrier = await addOrgCarrier(orgId, carrierData);
  ...
  useEffect(() => {
    ...
    if (isNew && carrierName) {
      handleAddCarrierOptions(orgId, carrierName);
    }
  }, [watchCarrier]);
